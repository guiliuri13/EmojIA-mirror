export default class GestureDescription {
	public name: string;
	public curls: { [finger: number]: [number, number][] };
	public directions: { [finger: number]: [number, number][] };
	...
	addCurl(finger: number, curl: number, contrib = 1.0) {
	...
	addDirection(finger: number, position: number, contrib = 1.0) {
	...
	matchAgainst(detectedCurls: string[], detectedDirections: string[]) {
		let score = 0.0;
		let numParameters = 0;
		...
		console.log("🚀 ~ file: GestureDescription.ts:103 ~ GestureDescription ~ matchAgainst ~ finalScore:", finalScore)
		return finalScore;
	}
}	