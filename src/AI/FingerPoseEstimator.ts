...
export default class FingerPoseEstimator {
	options: {
		// curl estimation
		HALF_CURL_START_LIMIT: number; NO_CURL_START_LIMIT: number;
		// direction estimation
		DISTANCE_VOTE_POWER: number; SINGLE_ANGLE_VOTE_POWER: number; TOTAL_ANGLE_VOTE_POWER: number;
	};
	...
	estimate(landmarks: { [x: number]: any; }): { curls: any; directions: any; } {
		...
		for (let finger of Finger.all) {
			...
			for (let point of points!) {
				...
				let slopes = this.getSlopes(point1, point2) as number[];
		...
		for (let finger of Finger.all) {
			...
			if (!fingerPointsAt) return { curls: fingerCurls, directions: fingerDirections};
